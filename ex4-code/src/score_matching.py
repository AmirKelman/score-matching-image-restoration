...
class ScoreMatchingModel(nn.Module):
    ...
    def forward(self, x, t):
        t = torch.full((x.size(0),), t, device=x.device, dtype=torch.long)  # Ensure t is a tensor of the correct size
        return self.nn_module(x, t)
    ...
    @torch.no_grad()
    def sample(self, bsz, device, noise = 1.0, x0 = None):
        """
        Parameters
        ----------
 ...
 ###### for Q1 ######
        ...
        #     )
        #     samples[idx+1] = x * normalization_factor
        #
        # return samples
...
###### for Q2 ######
        ...
        sigma_start = torch.zeros((bsz,), device=device)
        sigma_end = torch.zeros((bsz,), device=device)

        if x0 is None:
            x_t = torch.randn((bsz, *self.input_shape), device=device) * sigmas[0]
        else:
        ...
        for idx in range(num_sampling_timesteps):

            print("idx: ", idx)
            sigma_start.fill_(sigmas[idx])
            sigma_end.fill_(sigmas[idx+1])

            pred_x_t = self.nn_module_wrapper(samples[idx], sigma_start)

            noise_t = torch.randn_like(samples[idx]) * sigma_end
            samples[idx+1] = pred_x_t + noise_t
